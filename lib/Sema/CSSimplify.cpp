...
ConstraintSystem::matchExistentialTypes(Type type1, Type type2,
  ...
  if (layout.isNarrowedAny()) {
    ...
    //
    // Note: this includes "subset → superset" (`E1 | E2 →
    // E1 | E2 | E3`). The proposal makes no carve-out for it: even a
    // strictly wider narrowed-Any has different synthesised Codable
    // try-order / switch-completion / mangled name / protocol-witness
    // selection, so users must spell the widen with `as`. Chained
    // typed-throws widening is therefore done by wrapping the inner
    // call in a closure that explicitly throws-as the wider shape.
